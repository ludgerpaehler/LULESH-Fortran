...
CONTAINS
  ...
  SUBROUTINE AllocateNodeElemIndexes(domain)
    ...
    !CHECK THIS LOOP - THINK IT'S OK ???? -> Comment from original write of lulesh.f90
...
!$OMP END PARALLEL DO
  ...
  SUBROUTINE CalcCourantConstraintForElems(domain)
    ...
    ! OMP region works with the following:
    !  !$OMP DO
    DO i = 0, length-1
  ...
  SUBROUTINE CalcHydroConstraintForElems(domain)
    ...
    !  !$OMP DO PARALLEL FIRSTPRIVATE(length, dvovmax) PRIVATE(indx, thread_num, i, dthydro_tmp, hydro_elem)
